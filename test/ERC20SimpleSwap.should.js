const { BN, balance, time, expectEvent, expectRevert } = require('@openzeppelin/test-helpers');
...
function shouldCashChequeBeneficiary(recipient, cumulativePayout, signee, from) {
  beforeEach(async function () {
    ...
    const { logs, receipt } = await this.ERC20SimpleSwap.cashChequeBeneficiary(recipient, cumulativePayout, issuerSig, {
      from: from,
    });
    this.logs = logs;
...
function shouldNotCashChequeBeneficiary(
  ...
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.cashChequeBeneficiary(recipient, toSubmitCumulativePayout, this.issuerSig, {
        from: from,
        value: value,
      }),
      revertMessage
...
function shouldCashCheque(
  ...
  beforeEach(async function () {
    ...
    const { logs, receipt } = await this.ERC20SimpleSwap.cashCheque(
      beneficiary,
      recipient,
      cumulativePayout,
      beneficiarySig,
...
function shouldNotCashCheque(
  ...
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.cashCheque(
...
function shouldPrepareDecreaseHardDeposit(beneficiary, decreaseAmount, from) {
  ...
  it('should update the canBeDecreasedAt', async function () {
    ...
    expect(this.postconditions.hardDepositFor.canBeDecreasedAt.toNumber()).to.be.closeTo(
      (await time.latest()).add(expectedCanBeDecreasedAt).toNumber(),
      5
    );
  });
...
function shouldNotPrepareDecreaseHardDeposit(beneficiary, decreaseAmount, from, value, revertMessage) {
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.prepareDecreaseHardDeposit(beneficiary, decreaseAmount, { from: from, value: value }),
      revertMessage
    );
  });
}
...
function shouldNotDecreaseHardDeposit(beneficiary, from, value, revertMessage) {
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.decreaseHardDeposit(beneficiary, { from: from, value: value }),
      revertMessage
    );
  });
}
...
function shouldNotIncreaseHardDeposit(beneficiary, amount, from, value, revertMessage) {
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.increaseHardDeposit(beneficiary, amount, { from: from, value: value }),
      revertMessage
    );
  });
}
function shouldSetCustomHardDepositTimeout(beneficiary, timeout, from) {
  beforeEach(async function () {
    ...
    const { logs } = await this.ERC20SimpleSwap.setCustomHardDepositTimeout(beneficiary, timeout, beneficiarySig, {
      from: from,
    });
    this.logs = logs;
...
function shouldNotSetCustomHardDepositTimeout(toSubmit, toSign, signee, from, value, revertMessage) {
  ...
  it('reverts', async function () {
    await expectRevert(
      this.ERC20SimpleSwap.setCustomHardDepositTimeout(toSubmit.beneficiary, toSubmit.timeout, this.beneficiarySig, {
        from: from,
        value: value,
      }),
      revertMessage
...
function shouldNotWithdraw(amount, from, value, revertMessage) {
  it('reverts', async function () {
    await expectRevert(this.ERC20SimpleSwap.withdraw(amount, { from: from, value: value }), revertMessage);
  });
}
