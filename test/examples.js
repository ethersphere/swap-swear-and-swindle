...
contract("MailboxTrial", function(accounts) {
  ...
  it("should allow swap if guilty", async () => {
    ...
    let trial = await MailboxTrial.new(swindle.address, updateWitness.address, merkleWitness.address);
    ...
    var { receipt, logs: [{ args: { caseId} }] } = await swindle.startTrial(contractor, client, trial.address, await trial.encodeInitialPayload(500))
    
    var { receipt } = await swindle.continueTrial(caseId, await trial.encodePayloadSwearNote(serviceNoteEncoded, serviceNote.sig))
    ...
    var { receipt } = await swindle.continueTrial(caseId, await trial.encodePayloadSwearNote(paymentNoteEncoded, paymentNote.sig))
    
    let ackWitness = await AckWitness.at(await trial.ackWitness())    
    ...
    var { receipt } = await swindle.continueTrial(caseId, await trial.encodePayloadSwearNote(ackEncoded, await web3.eth.sign(ackHash, contractor)))
    ...
    var { receipt } = await swindle.continueTrial(caseId, await trial.encodePayloadSwearNote(updateEncoded, await web3.eth.sign(updateHash, contractor)))

    await time.increase(3600*24*30)

    await swindle.continueTrial(caseId, "0x")
  ...
  it("should not allow swap if not guilty", async () => {
    ...
    var { receipt } = await swindle.continueTrial(caseId, encodedProof);
